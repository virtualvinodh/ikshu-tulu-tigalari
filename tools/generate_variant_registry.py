...
registry["_conjunct_variants_readme"] = (
    ...
    "3+-consonant conjuncts (e.g. ka_sa_sa-tutg) and compound consonant+vowel-"
    "sign ligatures (e.g. ka_uMatra-tutg) are picked up by this script's alt-"
    "glyph scan but deliberately left out of this section and reported in "
    "VARIANT_REGISTRY_TODO.md instead - seeing this content below any of it."
)
conjunct_out = {}
...
with open(report_path, "w", encoding="utf-8") as f:
    ...
    if skipped_unclassified:
        ...
        f.write("Not consonant/vowel/vowel-sign/mark, and not a plain two-consonant conjunct - "
                "mostly compound consonant+vowel-sign ligatures (e.g. `ka_uMatra-tutg`) or "
                "3+-consonant conjuncts (e.g. `ka_sa_sa-tutg`). Neither `generate_akhn_feature.py` "
                "nor `generate_blwf_feature.py` currently has a mechanism wired up for these - "
                "listed here for a future decision, not included in variant_registry.json.\n\n")
        for base in bases:
