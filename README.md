...
each — e.g. `taChillu-tutg` is excluded because its name implies TA + Looped Virama
but its only component is `ya-tutg.below`, a confirmed name/artwork mismatch, not a
renameable typo), `names_override.json` (every non-standard/typo/shorthand glyph-name
token `generate_akhn_feature.py` has to special-case — typo aliases like `UuMatra`
and the `XChillu` abbreviation table, each with its reason — see its own `_readme`
key; written so that manually renaming a glyph in the font to its correct standard
form keeps working with no code change, since standard names are always checked
first), `sample_texts_source.json` (the sample-text passages themselves).
